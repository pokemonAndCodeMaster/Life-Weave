 ...
 <template v-if="tab==='documents'">
  ...
  <div class="lw-knowledge-grid">
   ...
   <article class="lw-panel lw-article">
    ...
    <template v-else-if="selected"><div class="lw-between"><span class="lw-small lw-muted">{{ selected.sourceTitle }} · {{ selected.writable?'工作台管理':'外部只读来源' }}</span><div class="lw-inline"><button class="lw-btn sm" @click="downloadText(selected!.content,selected!.path.split('/').pop()||'知识.md')">下载</button><button class="lw-btn sm" @click="edit()">提出修订</button></div></div><div @click="follow"><MarkdownBody :content="selected.content"/></div><p class="lw-tiny lw-muted">{{ selected.path }} · 版本 {{ selected.version.slice(0,12) }}</p></template>
