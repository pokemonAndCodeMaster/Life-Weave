...
<template>
<PageHeader title="Linear 事项" subtitle="引入已有工作，查看来源，在发送成果前确认正文。"><RouterLink class="lw-btn" :to="`/lifeweave/${activeWorkspace}/settings`">连接设置</RouterLink><button class="lw-btn" :disabled="busy" @click="refresh()">刷新</button></PageHeader>
...
<section class="lw-panel pad lw-mt-20"><h2>把成果发回原事项</h2><p class="lw-small lw-sub">先预览，再发送为原事项的一条评论；不改写原来的目标或状态。</p><form class="lw-stack" @submit.prevent="prepare"><label class="lw-label">关联工作<select v-model="form.itemId" class="lw-field" required><option value="">选择工作</option><option v-for="entry in bindings" :key="entry.item_id" :value="entry.item_id">{{entry.remote_snapshot.identifier}} · {{entry.remote_snapshot.title}}</option></select></label><label class="lw-label">成果与下一步<textarea v-model="form.body" class="lw-field" rows="5" required placeholder="完成了什么、证据在哪里、还有哪些未完成……"></textarea></label><button class="lw-btn" :disabled="busy">生成发布预览</button></form><div v-if="publication" class="lw-publication-preview"><h3>发送到 {{bindings.find(b=>b.item_id===publication?.item_id)?.remote_snapshot.identifier||'关联事项'}} 的固定正文</h3><MarkdownBody :content="publication.body"/><button v-if="publication.status!=='confirmed'" class="lw-btn primary" :disabled="busy" @click="publish">确认发送到原事项</button><p v-else class="lw-small">已发送并回读确认。</p></div></section>
</template>
