...
import { apiError, getMeetingMarkdown, getMeetingPreview, getMeetingSnapshot } from '../api/lifeweave'
...
const { activeWorkspace, state, itemById, openModal, toggleMeetingSnapshot, resumeMeetingSnapshot, toggleMeetingSeen, notify } = useLifeWeaveWorkspace()
...
<template>
  <div :class="{ 'lw-presenting': presenting }">
    <PageHeader :title="isTeam ? '质检团队 · 周度组会' : '我的周回顾'" subtitle="把议程保存成呈现方式，不再手工搬运一遍进展。">
      <button class="lw-btn" type="button" @click="openModal('agenda-config')">配置议程</button><button class="lw-btn primary" type="button" :disabled="!sections.length" @click="presenting = true"><LifeWeaveIcon name="play" />开始演示</button>
    </PageHeader>
    <div class="lw-meeting-controls"><StatusBadge :value="meeting?.snapshot ? '已冻结本次内容' : '读取当前工作事实'" :tone="meeting?.snapshot ? 'green' : 'blue'" /><span class="lw-tiny lw-muted">{{ meeting?.snapshot ? `快照时间 ${meeting.snapshot.time}` : '当前实时视图' }}</span><span class="lw-spacer"></span><button class="lw-btn sm" type="button" :disabled="!meeting?.version" @click="toggleSnapshot">{{ meeting?.snapshot ? '回到实时内容' : '冻结本次内容' }}</button><button class="lw-btn sm" type="button" :disabled="!meeting?.version" @click="exportMarkdown"><LifeWeaveIcon name="download" />导出纪要</button></div>
    <div class="lw-meeting-config">
      <aside class="lw-panel pad lw-agenda-sidebar"><div class="lw-eyebrow">本次议程</div><button v-for="(section, position) in sections" :key="section.key" class="lw-agenda-button" :class="{ active: index === position }" type="button" @click="index = position"><span class="num">{{ String(position + 1).padStart(2, '0') }}</span>{{ section.title }}</button><hr class="lw-rule" /><p class="lw-small lw-sub">同一事项只在一个板块完整展开。专题保留目标、缺口与关联决定。</p><div class="lw-tiny lw-muted">已讨论 {{ meeting?.seen.length ?? 0 }} 项 · {{ meeting?.notes.length ?? 0 }} 条会中记录</div></aside>
      <section>
        <article v-if="currentSection" class="lw-slide-page"><div class="lw-slide-eyebrow">{{ isTeam ? 'QC TEAM / WEEKLY ALIGNMENT' : 'PERSONAL / WEEKLY REVIEW' }} · 工作事实</div><h2>{{ currentSection.title }}</h2><p class="lw-slide-desc">{{ descriptions[currentSection.key] ?? '按照已保存的会议配置读取当前事实。' }}</p>
          <template v-for="group in currentEntryGroups" :key="group.label">
            <div v-if="group.showHeading" class="lw-meeting-group-head"><span>{{ group.label }}</span><small>{{ group.entries.length }} 项</small></div>
            <div v-for="entry in group.entries" :key="`${entry.presentation}-${entryId(entry)}`" class="lw-meeting-item" :class="{ seen: entry.itemId && meeting?.seen.includes(entry.itemId) }"><div class="lw-between"><h3><span class="lw-mono lw-muted">{{ entryId(entry) }}</span> {{ String(entry.values.title || itemById(entry.itemId || '')?.title || '') }}</h3><StatusBadge :value="entry.itemId && meeting?.seen.includes(entry.itemId) ? '本次已讨论' : entry.presentation === 'full' ? '完整展开' : entry.presentation === 'decision' ? '语境决定' : entry.presentation === 'topic_summary' ? '专题摘要' : '摘要'" :tone="entry.itemId && meeting?.seen.includes(entry.itemId) ? 'green' : entry.presentation === 'decision' ? 'amber' : undefined" /></div><p v-for="field in displayFields(entry)" :key="field.key"><strong>{{ field.label }}：</strong>{{ field.value }}</p><div v-for="(decision, decisionIndex) in entry.decisions" :key="decisionIndex" class="lw-notice neutral"><strong>语境决定：</strong>{{ displayValue(decision.body ?? decision.title) }}</div><div v-if="entry.itemId" class="lw-between"><span class="lw-tiny lw-sub">由会议投影按去重规则生成</span><div class="lw-inline"><button v-if="itemById(entry.itemId)" class="lw-btn sm" type="button" @click="openModal('peek', { item: itemById(entry.itemId) })">查看依据</button><button v-if="itemById(entry.itemId)" class="lw-btn sm" type="button" @click="openModal('meeting-note', { item: itemById(entry.itemId), snapshotId: meeting?.snapshot?.snapshotId })">记录决定</button><button class="lw-btn sm" type="button" @click="toggleMeetingSeen(entry.itemId)">{{ meeting?.seen.includes(entry.itemId) ? '撤销已讨论' : '标记已讨论' }}</button></div></div></div>
          </template>
          <div v-if="!currentSection.entries.length" class="lw-empty">本板块没有剩余事项。前面已经展开的内容不再重复。</div>
          <div class="lw-slide-footer"><span>LifeWeave · 同一事实，按议程呈现</span><span>{{ index + 1 }} / {{ keys.length }} {{ meeting?.snapshot ? '· 已冻结' : '· 实时视图' }}</span></div>
        </article>
        <div v-else class="lw-panel lw-empty">尚未配置会议议程。请先保存至少一个板块。</div>
        <div v-if="previewError" class="lw-notice warning lw-mt-14">{{ previewError }}</div>
        <div class="lw-between lw-meeting-standard-nav"><button class="lw-btn" type="button" :disabled="index === 0" @click="previous">← 上一页</button><span class="lw-tiny lw-muted">专题引用不会增加新的交付。</span><button class="lw-btn" type="button" :disabled="index >= keys.length - 1" @click="next">下一页 →</button></div>
        <div class="lw-presentation-nav"><button class="lw-btn" type="button" @click="previous">←</button><span class="lw-small lw-sub">← → 切页 · Esc 退出</span><button class="lw-btn" type="button" @click="next">→</button><button class="lw-btn" type="button" @click="presenting = false">退出演示</button></div>
      </section>
    </div>
    <div class="lw-meeting-bottom-note lw-notice neutral"><LifeWeaveIcon name="meeting" /><div>呈现配置决定看哪些对象、按什么顺序、展开到哪一层。冻结保存当时事实；会中记录关联原事项。</div></div>
  </div>
</template>
