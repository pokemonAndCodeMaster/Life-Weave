...
vi.mock('../api/library', () => ({
  links: vi.fn().mockResolvedValue({
    ...
    outgoing: [
      ...
      { sourceId: 'lifeweave-project', path: null, label: '旧档案', href: 'history/old.md', status: 'archived', externalUrl: 'https://example.test/blob/main/docs/history/old.md', count: 1 },
    ],
...
describe('KnowledgeRelations', () => {
  it('lets readers follow current Markdown relations while marking broken and blocked targets', async () => {
    ...
    expect(wrapper.text()).toContain('仓库档案')
    const links = wrapper.findAll('a')
    expect(links.map(link => link.text())).toEqual(['Drive1.0', '旧档案 （仓库档案）', '研究索引'])
    expect(links[0]?.attributes('href')).toContain('Drive1.0.md')
    expect(links[1]?.attributes('href')).toContain('https://example.test/blob/main/docs/history/old.md')
    expect(links[2]?.attributes('href')).toContain('索引.md')
  })
})
