...
watch(type, async (next, _previous, onCleanup) => {
  ...
  if (next === 'delegate' && item.value) {
    ...
    try {
      const [m, d, recommended] = await Promise.all([http.get(`/lifeweave/${workspace}/methods`), http.get(`/lifeweave/${workspace}/library/documents`), getRecommendations(workspace, id)])
...
<template>
  <LifeWeaveModal v-if="type" :title="payloadTitle" :wide="['run-detail', 'artifact', 'capability-detail'].includes(type)" @close="closeModal">
    ...
    <template v-else-if="type === 'capability-detail'"><div v-if="!capability" class="lw-empty">正在读取候选…</div><template v-else><div class="lw-inline"><StatusBadge :value="capability.target" /><StatusBadge :value="capability.status" /></div><h3 class="lw-mt-15">{{ capability.title }}</h3><p>{{ capability.desiredBehavior }}</p><div class="lw-prop"><span>候选版本</span><div class="lw-mono">{{ capability.version }}</div></div><div class="lw-prop"><span>验证计划</span><div>{{ capability.validationPlan }}</div></div><h3>已有验证</h3><div v-for="check in capability.verifications" :key="check.id" class="lw-note-card"><StatusBadge :value="check.result" /><p>{{ check.assessment }}</p><span class="lw-mono lw-tiny">{{ check.runId }} / {{ check.evidenceId }}</span></div><div v-if="capability.status !== 'published'"><hr class="lw-rule" /><div class="lw-form-grid"><label class="lw-label">真实 Run ID<input v-model="form.runId" class="lw-field" /></label><label class="lw-label">已接受证据 ID<input v-model="form.evidenceId" class="lw-field" /></label></div><label class="lw-label">评估结论<textarea v-model="form.assessment" class="lw-field"></textarea></label><label class="lw-label">验证判断<select v-model="form.result" class="lw-field"><option value="accepted">接受</option><option value="rejected">拒绝</option></select></label></div></template></template>
