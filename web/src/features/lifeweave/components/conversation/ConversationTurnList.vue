  ...
  <div class="turn-list" aria-label="对话记录">
    <article v-for="turn in rows" :key="turn.id" class="turn">
      ...
      <div class="assistant-message">
        ...
        <details v-if="turn.sources?.length" class="sources"><summary>本次参考来源（{{ turn.sources.length }}）</summary><ul><li v-for="(source, index) in turn.sources" :key="index"><a v-if="typeof source.url==='string'" :href="source.url">{{source.title}}</a><template v-else>{{ source.title || source.path || source.ref || '已记录来源' }}</template><span v-if="source.version"> · 版本 {{String(source.version).slice(0,12)}}</span><span v-if="source.excerpt"> · 节选</span><span v-if="source.path && source.title"> · {{ source.path }}</span></li></ul></details>
