...
const Harness = defineComponent({
  components: { LifeWeaveModal },
  setup() { const open = shallowRef(false); return { open } },
  template: `<button data-testid="trigger" @click="open = true">打开</button><LifeWeaveModal v-if="open" title="键盘测试" @close="open = false"><input aria-label="第一项" autofocus /><button>末项</button></LifeWeaveModal>`,
})

describe('LifeWeaveModal', () => {
