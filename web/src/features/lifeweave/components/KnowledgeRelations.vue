...
<template>
  <section class="knowledge-relations" aria-label="知识引用关系">
    ...
    <template v-else-if="relations">
      <div class="relations-grid"><div><h4>本文引用</h4><ul v-if="relations.outgoing.length"><li v-for="entry in relations.outgoing" :key="entry.href + entry.status"><RouterLink v-if="entry.status === 'valid' && entry.path" :to="destination(entry.sourceId, entry.path)">{{ entry.label }}</RouterLink><a v-else-if="entry.status === 'archived' && entry.externalUrl" :href="entry.externalUrl" target="_blank" rel="noopener noreferrer">{{ entry.label }} <small>（仓库档案）</small></a><span v-else>{{ entry.label }} <small>（{{ entry.status === 'missing' ? '目标文件不存在' : '超出知识目录或不可读取' }}）</small></span><span v-if="entry.count > 1" class="lw-tiny lw-muted"> · {{ entry.count }} 次</span></li></ul><p v-else class="lw-small lw-sub">本文没有指向同一来源 Markdown 的链接。</p></div>
