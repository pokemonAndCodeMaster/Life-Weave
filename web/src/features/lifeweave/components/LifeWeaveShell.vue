...
const pageLabels: Record<string, string> = { conversation: '与经纬对话', plan: '计划与优先级', settings: '设置与连接', connections: 'Linear 事项', runs: 'AI 委托', home: '我的日常', items: '工作事项', ideas: '灵感与讨论', knowledge: '知识', meeting: '组会 / 回顾', maintenance: '维护中心', 'item-detail': '事项' }
...
const nav = computed(() => [
  { name: 'conversation', label: '与经纬对话', icon: 'message' },
...
<template>
  <div class="lifeweave-app">
    ...
    <aside class="lw-sidebar" :class="{ open: navOpen }">
      <div class="lw-brand"><img src="/favicon.svg" width="32" height="32" alt="" />经纬 <small>LifeWeave</small></div>
      ...
      <nav aria-label="经纬主要导航"><RouterLink v-for="entry in nav" :key="entry.name" class="lw-nav-item" :to="path(entry.name)" @click="navOpen = false"><LifeWeaveIcon :name="entry.icon" />{{ entry.label }}<span v-if="entry.name === 'home'" class="count">{{ rootItems.filter((item) => item.attention).length }}</span></RouterLink></nav>
      ...
      <div class="lw-sidebar-footer"><RouterLink class="lw-nav-item" :to="path('settings')"><LifeWeaveIcon name="settings" />设置与连接</RouterLink><div class="lw-profile"><span class="lw-avatar me">我</span><div><div class="lw-small lw-strong">{{ isTeam ? '团队成员 / 维护者' : '个人使用者' }}</div><div class="lw-tiny lw-sub">LifeWeave · 经纬</div></div></div></div>
    </aside>
