...
const labels:Record<string,string>={confirmed:'已归档并回读',pending:'等待归档',sending:'正在归档',failed:'归档失败',unconfigured:'未配置',historical_read_only:'历史只读'}
...
<template>
 <section class="archive-state lw-notice neutral" aria-label="归档状态">
  ...
  <p v-for="key in ['local','github','notion','linear']" :key="key" class="lw-small"><strong>{{key==='local'?'本机完整包':key==='github'?'GitHub':key==='notion'?'Notion 镜像':'Linear 历史'}}：</strong>{{labels[state[key]?.status||'pending']}} <a v-if="state[key]?.url" :href="state[key]!.url" target="_blank" rel="noopener noreferrer">打开归档</a><span v-if="state[key]?.error"> · {{state[key]!.error}}</span></p>
