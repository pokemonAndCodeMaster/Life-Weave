...
<template>
  <section class="lw-panel pad"><h2>推进记录</h2><p class="lw-small lw-sub">聚合有意义的变化、决定与运行；不要求所有工作经过同一套阶段。</p><div class="lw-timeline"><div v-for="activity in [...rootItem.activities].reverse()" :key="activity.id ?? `${activity.time}-${activity.title}`" class="lw-timeline-event"><time>{{ activity.time }}</time><h3>{{ activity.title }}</h3><p>{{ activity.text }}</p></div><div v-if="!rootItem.activities.length" class="lw-empty">事项已建立，等待下一动作。</div></div></section>
