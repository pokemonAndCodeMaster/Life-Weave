...
const props = defineProps<{ candidates: Candidate[]; items: Item[]; busy: boolean; template?: EvaluationTask | null }>()
const emit = defineEmits<{ create: [payload: { itemId: string; targetKind: 'system' | 'capability'; candidateId?: string; repeatOf?: string; title: string; instruction: string; criteria: string }]; clearTemplate: [] }>()
const form = reactive({ itemId: '', targetKind: 'system' as 'system' | 'capability', candidateId: '', title: '', instruction: '', criteria: '' })
...
const ready = computed(() => Boolean(form.itemId.trim() && form.title.trim() && form.instruction.trim() && form.criteria.trim() && (form.targetKind === 'system' || form.candidateId)))
function submit() {
...
<template>
  <form class="evaluation-form" @submit.prevent="submit">
    <p v-if="template" class="lw-notice neutral">沿用 {{ template.id }} 的同一任务与通过标准；可选择新的候选版本进行对照。<button class="lw-text-btn" type="button" @click="emit('clearTemplate')">改为新评测</button></p>
    <div class="evaluation-grid">
      ...
      <label class="lw-label">评测对象
        <select v-model="form.targetKind" class="lw-field" :disabled="Boolean(template)"><option value="system">整件事 / 平台能力</option><option value="capability">一项能力候选</option></select>
      </label>
    </div>
    ...
    <label class="lw-label">交给 AI 的任务<textarea v-model="form.instruction" class="lw-field" rows="3" required :disabled="Boolean(template)" placeholder="写清要完成什么；创建评测时不会启动 AI" /></label>
