...
function normalizeWorkspace(raw: WorkspaceState, workspace: WorkspaceKind): WorkspaceState {
  ...
  const displayStatus: Record<string, string> = { open: '待确认', planned: '已计划', in_progress: '进行中', blocked: '已阻塞', awaiting_acceptance: '待验收', completed: '已完成', cancelled: '已取消' }
  ...
  return {
    ...
    items: itemRows.map((rawItem: any) => {
      ...
      itemType: rawItem.itemType ?? 'other',
      ...
      artifacts,
      evidence,
      activities: (rawItem.activity ?? payload.activities ?? []).map((entry: any) => ({ id: entry.id, itemId: entry.itemId ?? rawItem.id, time: entry.createdAt ?? entry.time, title: entry.title ?? entry.kind ?? '事项更新', text: entry.text ?? entry.body ?? '', actor: entry.actorId, payload: entry.payload })),
...
async function loadItem(id: string, quiet = false) {
  ...
  if (!quiet) loading.value = true
  try {
    const detail = await getItemDetail(scope, id)
    if (scope !== activeWorkspace.value) return
    const normalized = normalizeWorkspace({
      workspace: activeWorkspace.value,
      items: [detail], ideas: state.value?.ideas ?? [], topics: state.value?.topics ?? [], domains: state.value?.domainEntities ?? [], resources: state.value?.resources ?? [], artifacts: state.value?.artifacts ?? [], knowledge: [], skills: [], improvements: state.value?.improvements ?? [], meeting: emptyMeeting(), preferences: {},
    } as any, activeWorkspace.value).items[0]
    if (normalized) itemDetails.value = { ...itemDetails.value, [id]: normalized }
    return normalized
  } catch (caught) {
    console.error('Failed to load LifeWeave item detail', caught)
    const problem = apiError(caught)
    error.value = problem
    notify(problem.message)
    return undefined
...
async function addFeedback(item: WorkItem, text: string, anchor: string, requestId: string) {
  const result = await mutate(`/items/${encodeURIComponent(item.id)}/feedback`, 'post', { body: text, anchor, requestId }, '纠偏已保存，下一轮运行会自动读取。')
  await loadItem(item.id, true)
  return result
}
...
export function useLifeWeaveWorkspace() {
  return {
    activeWorkspace,
    state,
    runs,
