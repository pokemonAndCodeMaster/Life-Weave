...
const pageLabels: Record<string, string> = { plan: '计划与优先级', settings: '设置与连接', connections: 'Linear 事项', runs: 'AI 委托', home: '我的工作', items: '工作事项', ideas: '灵感与讨论', knowledge: '知识', meeting: '组会 / 回顾', maintenance: '维护中心', 'item-detail': '事项' }
...
<template>
  <div class="gongzuo-app">
    ...
    <aside class="gz-sidebar" :class="{ open: navOpen }">
      ...
      <div class="gz-sidebar-footer"><RouterLink class="gz-nav-item" :to="path('settings')"><GongzuoIcon name="settings" />设置与连接</RouterLink><div class="gz-profile"><span class="gz-avatar me">我</span><div><div class="gz-small gz-strong">{{ isTeam ? '团队成员 / 维护者' : '个人使用者' }}</div><div class="gz-tiny gz-sub">共作 · 真实工作空间</div></div></div></div>
    </aside>
