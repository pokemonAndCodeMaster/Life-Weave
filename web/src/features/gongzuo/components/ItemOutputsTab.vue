...
<template>
<ManualResultEditor v-if="manualResult" :item="item" @close="manualResult=false" @saved="manualResult=false"/><div class="gz-between gz-mb-18"><p class="gz-small gz-sub">人工工作和 AI 委托的实际成果，都可以在这里审阅。</p><button class="gz-btn primary" @click="manualResult=true">登记工作成果</button></div>
  <div class="gz-artifact-grid">
    ...
    <article v-for="run in results" :key="run.id" class="gz-artifact-card"><div class="gz-inline gz-muted gz-mb-10"><GongzuoIcon name="file"/>AI 交付</div><h3>第 {{run.attempt}} 次委托结果</h3><p>{{run.instruction}}</p><button class="gz-btn" @click="openModal('run-detail',{runId:run.id})">阅读结果</button></article><article v-if="!item.artifacts.length && !results.length" class="gz-artifact-card"><div class="gz-inline gz-muted gz-mb-10"><GongzuoIcon name="file" />成果</div><h3>尚无成果回传</h3><p>委托运行或手工工作可以把真实产物关联到这里。</p><button class="gz-btn" type="button" @click="openModal('delegate', { item })">委托补充</button></article>
  </div>
