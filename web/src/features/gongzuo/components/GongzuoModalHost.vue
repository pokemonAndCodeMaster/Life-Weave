...
watch(type, async (next) => {
  ...
  if (next === 'delegate') { taskMethods.value=[];taskDocuments.value=[];try{const [m,d]=await Promise.all([http.get(`/lifeweave/${activeWorkspace.value}/methods`),http.get(`/lifeweave/${activeWorkspace.value}/library/documents`)]);taskMethods.value=m.data.items;taskDocuments.value=d.data.items}catch(e){modalError.value=apiError(e).message} }
