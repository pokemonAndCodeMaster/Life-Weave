...
watch([()=>form.body,type,activeWorkspace],async()=>{const generation=++searchGeneration;if(type.value!=='search'){searchDocuments.value=[];return}try{const response=await http.get(`/gongzuo/${activeWorkspace.value}/library/documents`,{params:{q:form.body}});if(generation===searchGeneration)searchDocuments.value=response.data.items}catch{if(generation===searchGeneration)searchDocuments.value=[]}})
function openDocument(doc:{path:string;sourceId:string}){closeModal();void router.push({path:`/gongzuo/${activeWorkspace.value}/knowledge`,query:{path:doc.path,source:doc.sourceId}})}
...
watch(type, async (next) => {
  ...
  if (next === 'delegate') { taskMethods.value=[];taskDocuments.value=[];try{const [m,d]=await Promise.all([http.get(`/gongzuo/${activeWorkspace.value}/methods`),http.get(`/gongzuo/${activeWorkspace.value}/library/documents`)]);taskMethods.value=m.data.items;taskDocuments.value=d.data.items}catch(e){modalError.value=apiError(e).message} }
...
<template>
  <GongzuoModal v-if="type" :title="payloadTitle" :wide="['run-detail', 'artifact', 'capability-detail'].includes(type)" @close="closeModal">
    ...
    <template v-else-if="type === 'artifact' && artifact"><StatusBadge :value="artifact.kind || '成果'" /><h2 class="gz-mt-16">{{ artifact.name || artifact.title }}</h2><p>{{ artifact.summary || artifact.description }}</p><MarkdownBody v-if="artifact.body" :content="artifact.body"/><div class="gz-prop"><span>固定版本</span><div class="gz-mono">{{ artifact.artifactVersion || artifact.version || '未提供' }}</div></div><div class="gz-prop"><span>产物地址</span><div class="gz-mono">{{ artifact.uri || '未提供' }}</div></div><a v-if="String(artifact.uri || '').startsWith('http')" class="gz-btn" :href="artifact.uri" target="_blank" rel="noreferrer">打开实际产物</a></template>
    <template v-else-if="type === 'evidence' && evidence"><h3>{{ evidence.name }}</h3><p>{{ evidence.purpose }}</p><MarkdownBody v-if="(evidence as any).payload?.body" :content="(evidence as any).payload.body"/><div class="gz-prop"><span>当前状态</span><StatusBadge :value="evidence.result" /></div><div class="gz-prop"><span>证据引用</span><div>{{ evidence.source }}</div></div><label class="gz-label">审阅原因<textarea v-model="form.reason" class="gz-field"></textarea></label><div class="gz-notice neutral"><GongzuoIcon name="shield" /><span>接受证据只确认这份固定产物、环境和范围；不会自动完成事项。</span></div></template>
    ...
    <template v-else-if="type === 'search'"><input v-model="form.body" class="gz-field" aria-label="搜索" autofocus placeholder="输入标题、ID 或进展……" /><div class="gz-search-results"><button v-for="result in searchResults" :key="`${result.kind}-${result.id}`" type="button" @click="result.kind === 'item' ? openItem(result.id) : notify(`${result.kind === 'idea' ? '灵感' : '资源'}：${result.label}`)"><span class="gz-mono gz-muted">{{ result.id }}</span> {{ result.label }}<StatusBadge :value="result.kind" /></button><button v-for="doc in searchDocuments" :key="doc.sourceId+doc.path" @click="openDocument(doc)">{{doc.title}}<StatusBadge value="知识"/></button><div v-if="!searchResults.length && !searchDocuments.length" class="gz-empty">没有找到匹配内容。</div></div><p class="gz-dialog-note">搜索当前空间的事项、灵感、资源及已接入知识正文。</p></template>
