...
async function refresh(more=false){
 ...
 try{
  ...
  for(let page=0;page<count;page++){
   const response=(await http.get<{items:GongzuoRun[];total:number}>(`/gongzuo/${scope}/runs`,{params:{limit:100,offset:more?runs.value.length:page*100,state:filter.value||undefined}})).data
