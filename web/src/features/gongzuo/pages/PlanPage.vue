...
const {state,activeWorkspace,openModal}=useGongzuoWorkspace()
...
<template><PageHeader title="计划与优先级" subtitle="先看正在推进的工作，再决定下一件值得做的事。"><button class="gz-btn primary" @click="openModal('item-create')">新建事项</button></PageHeader><div class="gz-toolbar"><div class="gz-segmented"><button :class="{active:mode==='stage'}" @click="mode='stage'">按工作阶段</button><button :class="{active:mode==='priority'}" @click="mode='priority'">按优先级</button></div><input v-model="query" placeholder="查找工作" aria-label="查找工作"/><label class="gz-small"><input v-model="showClosed" type="checkbox"/> 显示已结束</label><span class="gz-spacer"></span><span class="gz-small gz-muted">{{filtered.length}} 件工作</span></div><div class="gz-board"><section v-for="group in groups" :key="group.label" class="gz-board-column"><header><h2>{{group.label}}</h2><span>{{group.items.length}}</span></header><article v-for="item in group.items" :key="item.id" class="gz-plan-card"><RouterLink :to="`/lifeweave/${activeWorkspace}/items/${item.id}/overview`">{{item.title}}</RouterLink><p>{{item.update||item.goal}}</p><div class="gz-between gz-small"><span :class="{'gz-overdue':overdue(item)}">{{item.due||'未安排日期'}}{{overdue(item)?' · 已逾期':''}}</span><button class="gz-text-btn" @click="selected=item">安排</button></div><span v-if="item.parentId" class="gz-tiny gz-muted">子工作 · {{item.owner}}</span></article><p v-if="!group.items.length" class="gz-board-empty">暂无事项</p></section></div><WorkPlanEditor v-if="selected" :key="selected.id" :item="selected" @close="selected=null" @saved="selected=null"/></template>
