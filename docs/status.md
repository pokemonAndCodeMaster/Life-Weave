...
| 查看开发插件目录、按空间停用新调用，并在事项对照固定计划与真实受管调用 | [插件实现说明](plugin-system.md)；一次性数据库回归覆盖绑定、停用、跨空间与执行边界，正式库已备份并迁移 | 插件级评价、知识语义关系和 Notion 自动镜像尚未接通；此切片尚待正式 Codex 自用和页面核查，不等于整份阶段 1 方案已完成 |
