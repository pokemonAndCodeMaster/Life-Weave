...
| 部分 | 章节 | 来源 | 原文 SHA-256 |
| --- | --- | --- | --- |
| 当前说明 | [LifeWeave](#doc-01) | `README.md` | `5d4399ba71283a48aa413a4af43cba67f6722d850a8480f0745bbfa10bbcee13` |
...
| 当前说明 | [产品设计：让分散的事情接得上、推得动](#doc-04) | `docs/product.md` | `689b3f92a1efc0e49fd01987376457044c75a530cbb6bf628d4fbe8b2324ce33` |
| 当前说明 | [架构与关键实现](#doc-05) | `docs/architecture.md` | `c97daaf7a176fded225187cc61a4f713ab402982a089b9bedbebdcb9d6cccdc4` |
| 当前说明 | [当前完成情况](#doc-06) | `docs/status.md` | `3fffbbecaf3fc2e6e243184d63b6fc6916ae4d355e548f97d2a8fba53993995b` |
...
| 建设方案（含未实现范围） | [LifeWeave 平台演进：原始诉求、已实现能力与差距](#doc-11) | `workspaces/reviews/personal-platform-evolution/review.md` | `d83915fef239a6b7f7cadc4dc80f7f1c93397757dfca36d57ec369e8a724ccf8` |
...
| 历史与验证附录 | [个人管理平台优化：本轮验证](#doc-17) | `docs/evidence/personal-platform-evolution/README.md` | `577d67e6e95629cace6cb1724199eaf8f1355470ce62dfa7c50baadf4fcb028d` |
...
| 评测任务、能力历史与改进接续 | 可直接定义 Skill/Agent/Harness 候选，或从改进建议建立；从现有事项建立平台整体或能力候选的任务与标准；启动时固定仓库、方法、知识，读过程、接受证据后评判；候选详情显示工作样例、失败历史及当前候选的全部关联委托；可沿原标准再评或把发现转为有来源的改进建议 | 2026-09-26 临时库 API/迁移纵切覆盖状态门槛、空间隔离、非空输入快照、同标准与并发发布阻断、显式前任候选的失败复测、候选历史与委托分页及建议去重。两次本机真实 Codex 样例均成功；第二次实际选入方法、知识和 Agent 候选，输出引用了所选原文，实施者按窄标准接受证据并评为通过。浏览器已打开该候选历史和直接创建入口，普通委托候选选择器已能选中候选。没有发布该内部候选，也不能由此宣布跨领域模型质量或用户验收 |
...
<a id="doc-11-line-103"></a>

### 直接创建候选与当前未做的关键目标

“能力与成长”新增“创建候选”入口，可直接填写 Skill、Agent 配置或 Harness 的真实内容、预期行为和验证计划；原有从改进建议进入的路径继续保留来源关联。跨类型改进会保留建议来源，但不把旧候选误作同类型前任；未编辑的 Skill 模板在切换类型时换成建议正文，已编辑内容保留并提示复核。创建只得到待试验候选，不会安装或发布；还需绑定事项真实运行、接受证据、按可见评测标准判断后显式发布。[直接创建页面](evidence/personal-platform-evolution/capability-create.png)已在本机浏览器打开，无运行时错误。另起独立临时数据库和 8011 服务后，浏览器从该页面提交无来源 Agent，API 回读 `candidate`，刷新页面仍有该候选；验收后删除临时库，未在个人正式空间额外创建演示候选。发布门槛仍由临时库 API 回归支持。
...
| 手动建立能力候选 | “能力与成长”点击“创建候选”打开[真实页面](evidence/personal-platform-evolution/capability-create.png)，可见名称、类型、完整内容、期望行为和验证计划；默认 Agent 配置为空正文，切到 Skill 会给出可编辑模板，再切回 Agent 会清除未修改的模板。独立临时数据库的 8011 页面实际提交一个无来源 Agent，API 回读 `candidate` 且无来源/前任 ID，刷新页面仍显示候选；无脚本错误，随后停止临时服务并删除临时库。组件回归进一步验证同类型继承，以及跨类型改进仅保留建议来源且清掉未编辑的 Skill 模板。后端在临时库已验证 Skill 内容校验和发布门槛。 | 未在正式个人库提交演示候选。创建不等于安装或发布；临时库的候选不保留。 |
