...
async def main():
 d=json.loads((E/'qwen-drive-start.json').read_text());tid=d['turn']['itemId'];rid=d['turn']['runId'];end=time.time()+2400;last=''
 while time.time()<end:
 ...
 (E/'qwen-drive-result.json').write_text(json.dumps(record,ensure_ascii=False,indent=2))
 ...
 async with async_playwright() as p:
  ...
  await page.goto(U+'/lifeweave/personal/items/'+tid+'/outputs');await page.get_by_role('heading',name='当前成果',exact=True).wait_for();await page.screenshot(path=str(E/'qwen-drive-result.png'),full_page=True);await b.close()
