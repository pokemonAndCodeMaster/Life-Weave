...
P='knowledge-notice-final-image-'
...
temp=Path(tempfile.mkdtemp(prefix=P));db='test_notice_image_'+uuid4().hex[:12]
...
try:
    ...
    class Fixture:
        ...
        async def run(self,request,on_event,on_process):
            ...
            (request.worktree/'fake.png').write_text('This is not a PNG')
            (request.worktree/'escape.png').symlink_to('/etc/passwd')
    ...
    for row in data['runs']:
        rooturl=base+'/runs/'+row['runId']
        for label,url in [('cross-space',rooturl.replace('/personal/','/team/')+'/source?path=figure.png'),('fake PNG',rooturl+'/source?path=fake.png'),('parent escape',rooturl+'/source?path=../figure.png'),('symlink escape',rooturl+'/source?path=escape.png')]:
            response=c.get(url)
            check(row['name']+' '+label+' rejected',response.status_code in [400,404,409],{'url':url,'status':response.status_code,'body':response.text})
        response=c.get(rooturl+'/source?path=figure.png')
        asset=c.get(rooturl+'/assets?path=figure.png')
        check(row['name']+' ordinary image source matches asset owner',response.status_code==200 and response.content==asset.content==png and response.headers['content-type']=='image/png' and response.headers['x-content-type-options']=='nosniff',dict(response.headers))
    ...
    with sync_playwright() as pw:
        ...
        def click_image_link(view,scope,label,row):
            with view.expect_popup() as pending:
                scope.get_by_role('link',name='普通图链接',exact=True).click()
            imagepage=pending.value
            imagepage.wait_for_load_state()
            imagepage.wait_for_function('document.querySelector("img") && document.querySelector("img").naturalWidth===1')
            check(label+' ordinary PNG link click displays real image',row['runId'] in imagepage.url and '/source?path=figure.png' in imagepage.url,{'url':imagepage.url,'naturalWidth':imagepage.locator('img').evaluate('(e)=>e.naturalWidth')})
            imagepage.screenshot(path=str(E/(P+label+'-image-click.png')))
            imagepage.close()
        def create_candidate(row,path,merge=False):
            ...
            click_image_link(page,region,row['name']+'-output',row)
            ...
            if not merge:click_image_link(page,review,row['name']+'-candidate',row)
        ...
        def accept_and_export(row,path,candidate,review,label,expect_nul):
            ...
            if label!='overlap':
                for view,key in [(page,'formal'),(exported,'downloaded HTML')]:
                    click_image_link(view,view,label+'-'+key.replace(' ','-'),row)
                ...
                check('overlap ambiguity visible in exported HTML',sum('多个来源使用相同相对引用' in w for w in warnings)==3 and all(w in exported.locator('body').inner_text() for w in warnings))
        ...
        check('multiple runs keep NUL explanation and ambiguity',overlap['references']['warnings'].count(nul['output']['storageNote'])==1 and len(overlap['references']['warnings'])==4 and not overlap['references']['links'] and not overlap['references']['images'],overlap['references'])
