...
def save(): (E/'encoding-final-review.json').write_text(json.dumps(OUT,ensure_ascii=False,indent=2,default=str))
...
with sync_playwright() as pw:
 ...
 page.screenshot(path=str(E/'encoding-final-real-knowledge.png'),full_page=True)
 for label,name in [('下载原文','encoding-final-real-original.md'),('下载阅读版','encoding-final-real-reading.html')]:
  with page.expect_download() as down:page.get_by_role('button',name=label,exact=True).click()
  down.value.save_as(E/name)
 OUT['real_raw_download_unchanged']=(E/'encoding-final-real-original.md').read_text()==before['content']
 page.goto((E/'encoding-final-real-reading.html').as_uri());a=page.get_by_role('link',name='来源记录',exact=True);OUT['real_html_source']={'href':a.get_attribute('href'),'note':page.locator('body').inner_text()[:70]}
...
with tempfile.TemporaryDirectory(prefix='lifeweave-encoding-review-') as tmp:
 root=Path(tmp);name='test_lifeweave_encoding_'+uuid4().hex[:12]
...
'''
   ...
   for f in ('source.md','source one.md','来源.md','100%.md','literal%20.md','source(a).md'):(folder/'research'/f).write_text('# Source identity '+run['id']+'\n\n## section\nIndependent review content.')
   for f in ('figure.png','fig one.png','图.png','fig(a).png','100%.png','literal%20.png'):(folder/'research'/f).write_bytes(PNG)
   ...
   with sync_playwright() as pw:
    ...
    page.wait_for_timeout(300);OUT['candidate_browser']=observation(page);
    OUT['candidate_actual_clicks']={}
    for container_name,container in [('original',page.locator('.research-output')),('candidate',page.locator('.research-knowledge'))]:
     for label in ['带空格来源','中文来源','行号来源','百分号来源','字面编码来源','括号来源','引用式来源']:
      # Only one link per Markdown consumer, scoped by its surrounding panel.
      link=container.get_by_role('link',name=label,exact=True).first
      with page.expect_popup() as pop:link.click()
      popup=pop.value;popup.wait_for_load_state();OUT['candidate_actual_clicks'][container_name+':'+label]={'url':popup.url,'body':popup.locator('body').inner_text()};popup.close()
    OUT['original_image_sources']={f:c.get(f'/api/lifeweave/personal/runs/{run["id"]}/assets',params={'path':'research/'+f}).status_code for f in ['figure.png','fig one.png','图.png','fig(a).png']};page.screenshot(path=str(E/'encoding-final-candidate.png'),full_page=True)
    ...
    page.wait_for_timeout(350);OUT['accepted_browser']=observation(page);page.screenshot(path=str(E/'encoding-final-accepted.png'),full_page=True)
    OUT['source_clicks']={}
    ...
    OUT['actual_clicks']={}
    for label in ['普通来源','带空格来源','中文来源','行号来源','锚点来源','百分号来源','字面编码来源','括号来源','引用式来源']:
     with page.expect_popup() as pop:page.get_by_role('link',name=label,exact=True).click()
     popup=pop.value;popup.wait_for_load_state();OUT['actual_clicks'][label]={'url':popup.url,'body':popup.locator('body').inner_text()};popup.close()
    with page.expect_download() as down:page.get_by_role('button',name='下载阅读版').click()
    down.value.save_as(E/'encoding-final-controlled-reading.html')
    with page.expect_download() as down:page.get_by_role('button',name='下载原文').click()
    down.value.save_as(E/'encoding-final-controlled-original.md')
    doc=s.library.document('personal','local',path);OUT['raw_unchanged']=doc['content']==candidate['content']==(E/'encoding-final-controlled-original.md').read_text();OUT['accepted_version']=doc['version']
    ...
    page.goto((E/'encoding-final-controlled-reading.html').as_uri());
    for image in page.locator('img').all():image.scroll_into_view_if_needed()
    page.wait_for_timeout(500);OUT['download_browser']=observation(page)
    OUT['download_actual_clicks']={}
    for label in ['带空格来源','中文来源','行号来源','百分号来源','字面编码来源','括号来源','引用式来源']:
     with page.expect_popup() as pop:page.get_by_role('link',name=label,exact=True).click()
     popup=pop.value;popup.wait_for_load_state();OUT['download_actual_clicks'][label]={'url':popup.url,'body':popup.locator('body').inner_text()};popup.close()
    ...
    page.goto(base+'/lifeweave/personal/knowledge');page.get_by_role('button',name='修订与历史').click();page.locator('.lw-knowledge-nav button').filter(has_text='已接受').filter(has_text=path).click();page.get_by_text('阅读建议全文',exact=True).click();page.wait_for_timeout(300)
    for image in page.locator('img').all():image.scroll_into_view_if_needed()
    page.wait_for_timeout(300);OUT['library_revision_browser']=observation(page)
    OUT['revision_actual_clicks']={}
    for label in ['带空格来源','中文来源','行号来源','百分号来源','字面编码来源']:
     with page.expect_popup() as pop:page.get_by_role('link',name=label,exact=True).click()
     popup=pop.value;popup.wait_for_load_state();OUT['revision_actual_clicks'][label]={'url':popup.url,'body':popup.locator('body').inner_text()};popup.close()
   ...
   OUT['browser_errors']=BROWSER_ERRORS
   OUT['checks']={}
   for group in ['candidate_browser','accepted_browser','download_browser','library_revision_browser']:
    images=[i for consumer in OUT[group] for i in consumer['images']]
    OUT['checks'][group+'_images']={'count':len(images),'all_natural_width_positive':all(i['width']>0 and i['complete'] for i in images),'notices':[consumer['text'] for consumer in OUT[group] if '加载失败' in consumer['text']]}
   for group in ['candidate_actual_clicks','actual_clicks','download_actual_clicks','revision_actual_clicks']:
    OUT['checks'][group]=all(run['id'] in click['body'] and 'Independent review content.' in click['body'] for click in OUT[group].values())
   OUT['checks']['downstream_references_match']=caps[0]['references']==doc['references']==OUT['downstream_manifest'][0]['references']
   save()
