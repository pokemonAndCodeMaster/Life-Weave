...
def save_result(request: Request, workspace: WorkspaceKey, item_id: str, body: ManualResult):
    ...
    with db.transaction() as conn:
        item = conn.execute('SELECT id FROM workbench.t_gongzuo_item WHERE id=%s AND workspace_key=%s FOR UPDATE', (item_id, workspace)).fetchone()
        if not item:
            raise HTTPException(404, '工作事项不存在')
        payload = {'kind':'手工成果','body':body.content,'summary':body.verification,'artifactVersion':digest}
        conn.execute("INSERT INTO workbench.t_gongzuo_entity(id,workspace_key,entity_type,title,payload,created_by,updated_by) VALUES(%s,%s,'artifact',%s,%s,'local-user','local-user') ON CONFLICT(id) DO NOTHING", (artifact_id,workspace,body.title,Jsonb(payload)))
        conn.execute("INSERT INTO workbench.t_gongzuo_relation(id,workspace_key,from_kind,from_id,to_kind,to_id,relation_type,created_by) VALUES(%s,%s,'item',%s,'entity',%s,'produces','local-user') ON CONFLICT DO NOTHING", ('relation-'+identity, workspace,item_id,artifact_id))
        conn.execute("INSERT INTO workbench.t_gongzuo_evidence(id,workspace_key,item_id,artifact_ref,artifact_version,environment_ref,summary,payload,created_by) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,'local-user') ON CONFLICT(id) DO NOTHING", (evidence_id,workspace,item_id,artifact_id,digest,body.environment,body.verification,Jsonb({'title':body.title,'body':body.content})))
