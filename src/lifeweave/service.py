...
from .repository import ConcurrentUpdateError, LifeWeaveRepository
...
class LifeWeaveService:
    def __init__(self, repository: LifeWeaveRepository) -> None:
    ...
    def record_feedback(self, workspace: str, item_id: str, *, body: str, run_id: str | None, request_id: str, actor_id: str, anchor: str | None = None) -> dict[str, Any]:
        self.get_item(workspace, item_id)
        if not body.strip():
            raise ValueError('反馈不能为空')
        if run_id:
            run = self.repository.run_identity(workspace, run_id)
            if run is None or run['itemId'] != item_id:
                raise ValueError('反馈引用的运行不属于当前事项和空间')
        context = self.current_context_snapshot(workspace, item_id)
        identity = hashlib.sha256(json.dumps([workspace, item_id, request_id]).encode()).hexdigest()[:40]
        return self.repository.save_feedback(workspace, {'id': 'feedback-' + identity, 'item_id': item_id,
            'body': body.strip(), 'actor': actor_id,
            'payload': {'kind': 'execution_feedback', 'runId': run_id, 'requestId': request_id, 'anchor': anchor,
                        'contextVersionId': context['versionId']}})

    def execution_feedback(self, workspace: str, item_id: str) -> list[dict[str, Any]]:
        self.get_item(workspace, item_id)
        rows = self.repository.list_discussions(workspace, item_id=item_id)
        return [{'id': row['id'], 'body': row['body'], 'createdAt': row.get('createdAt'),
                 'createdBy': row.get('createdBy'), **row['payload']} for row in rows
                if row.get('payload', {}).get('kind') == 'execution_feedback']
