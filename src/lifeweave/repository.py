...
class LifeWeaveRepository:
    """Persistence owner for LifeWeave. All workspace scoping is enforced in SQL."""

    def __init__(self, postgres: PGConnector) -> None:
        self._postgres = postgres
        s = postgres.schema
        self.items = f'{s}.t_lifeweave_item'; self.entities = f'{s}.t_lifeweave_entity'; self.relations = f'{s}.t_lifeweave_relation'
        self.contexts = f'{s}.t_lifeweave_context'; self.versions = f'{s}.t_lifeweave_context_version'; self.proposals = f'{s}.t_lifeweave_context_proposal'
        self.discussions = f'{s}.t_lifeweave_discussion'; self.evidence = f'{s}.t_lifeweave_evidence'; self.activities = f'{s}.t_lifeweave_activity'
        self.preferences = f'{s}.t_lifeweave_preference'; self.meetings = f'{s}.t_lifeweave_meeting'; self.snapshots = f'{s}.t_lifeweave_meeting_snapshot'; self.notes = f'{s}.t_lifeweave_meeting_note'
        self.runs = f'{s}.t_lifeweave_run'
    ...
    def save_feedback(self, workspace: str, row: dict[str, Any]) -> dict[str, Any]:
        # Deterministic request identity also protects retries after an uncertain response.
        with self._postgres.transaction() as connection:
            connection.execute(f'INSERT INTO {self.discussions}(id,workspace_key,item_id,body,payload,created_by) VALUES (%(id)s,%(workspace)s,%(item_id)s,%(body)s,%(payload)s,%(actor)s) ON CONFLICT(id) DO NOTHING',
                               {**row, 'workspace': workspace, 'payload': Jsonb(row['payload'])})
            saved = connection.execute(f'SELECT * FROM {self.discussions} WHERE id=%s AND workspace_key=%s', (row['id'], workspace)).fetchone()
            if not saved or saved['body'] != row['body'] or any(saved['payload'].get(key) != row['payload'].get(key) for key in ('runId', 'anchor')):
                raise ConcurrentUpdateError('同一请求标识已用于不同反馈，请使用新的请求标识')
        return self._camel(saved)
