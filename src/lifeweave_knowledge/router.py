...
router=APIRouter(prefix='/api/lifeweave/{workspace}',tags=['lifeweave-knowledge'])
...
def call(request: Request, name: str, *args):
    try:
        return getattr(request.app.state.lifeweave_knowledge_service,name)(*args)
...
def candidates(request: Request, workspace: WorkspaceKey):
    service=request.app.state.lifeweave_knowledge_service
...
def candidate(request: Request, workspace: WorkspaceKey, candidate_id: str):
    try: return request.app.state.lifeweave_knowledge_service.repository.get(workspace,candidate_id)
