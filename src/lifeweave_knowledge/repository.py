...
class LifeWeaveKnowledgeRepository:
    def __init__(self, postgres: PGConnector) -> None:
        self.postgres = postgres
        self.table = f'{postgres.schema}.t_lifeweave_capability'
        self.checks = f'{postgres.schema}.t_lifeweave_capability_verification'
    ...
    def publish(self, workspace: str, candidate_id: str, version: str, actor: str) -> dict[str, Any]:
        with self.postgres.transaction() as conn:
            ...
            conn.execute('SELECT pg_advisory_xact_lock(hashtextextended(%s,0))',
                (f"lifeweave:{workspace}:{row['target']}:{row['source_path'] or row['title']}",))
