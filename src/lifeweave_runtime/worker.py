    ...
    def _request(self, suffix: str, payload: dict[str, Any]) -> Any:
        ...
        request = urllib.request.Request(
            ...
            headers={
                ...
                "X-LifeWeave-Worker-Token": self.token,
            },
            method="POST",
        )
        try:
    ...
    def _materialize_capabilities(self, run: dict[str, Any], worktree: Path) -> list[str]:
        ...
        for entry in run.get("capability_snapshot") or []:
            ...
            if target == "skill":
                ...
                destination = worktree / ".lifeweave" / "knowledge" / f"{capability_id}.md"
            else:
                destination = worktree / ".lifeweave" / "runtime-guidance" / f"{capability_id}.md"
            destination.parent.mkdir(parents=True, exist_ok=True)
            ...
            if target == "skill":
                ...
                # LifeWeave provenance belongs in the adjacent manifest.
                destination.write_text(content.rstrip() + "\n", encoding="utf-8")
            else:
                destination.write_text(
                    f"<!-- lifeweave: {status}; version: {entry.get('version') or 'unknown'} -->\n\n"
                    f"{content.rstrip()}\n",
                    encoding="utf-8",
                )
            if target == "skill":
                for relative, body in (entry.get("files") or {}).items():
                    support = (destination.parent / relative).resolve()
                    if not support.is_relative_to(destination.parent.resolve()) or Path(relative).is_absolute():
        ...
        if manifest:
            manifest_path = worktree / ".lifeweave" / "capability-manifest.json"
