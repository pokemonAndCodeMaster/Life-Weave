...
class LifeWeaveRuntimeService:
    def __init__(
        self,
        *,
        repository: LifeWeaveRuntimeRepository,
        lifeweave_service: Any,
    ...
    def _safe_branch(item_id: str, run_suffix: str) -> str:
        ...
        return f"lifeweave/{safe}-{run_suffix}"
    ...
    def create_run(
        ...
        *,
        ...
        self.lifeweave_service.append_activity(
            workspace,
            item_id,
            kind="run.created",
    ...
    def retry_run(
        ...
        *,
        ...
        item = self.lifeweave_service.get_item(workspace, str(old["item_id"]))
        context = (
            self.lifeweave_service.current_context_snapshot(workspace, str(old["item_id"]))
            if sync_context
        ...
        if sync_context and self.capability_provider:
            capabilities += [entry for entry in old.get("capability_snapshot", []) if entry.get("pinnedInput")]
        item = self._json_snapshot(item)
        context = self._json_snapshot(context)
        capabilities = self._json_snapshot(capabilities)
    ...
    def format_run(self, row: dict[str, Any]) -> dict[str, Any]:
        ...
        stale_context = False
        try:
            current = self.lifeweave_service.current_context_snapshot(
        ...
        if effective_outcome in TERMINAL_STATES:
            ...
            if artifacts:
                ...
                if artifact_ref and artifact_version and environment_ref:
                    self.lifeweave_service.add_run_evidence_link(
                        workspace,
                        str(row["item_id"]),
                        run_id=run_id,
