...
from .worker import LifeWeaveWorker, ServiceWorkerClient
...
class LocalWorkers:
    ...
    async def start(self, workspace):
        ...
        worker = LifeWeaveWorker(client=ServiceWorkerClient(self.runtime, workspace, identity['id'], identity['worker_token']), executors=self.engines, runtime_root=self.root/'.runtime/executions', machine_id=identity['id'], authentication_sources=authentication)
        async def loop():
            while True:
                try:
