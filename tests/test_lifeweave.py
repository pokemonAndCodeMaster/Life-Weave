...
from src.lifeweave.repository import ConcurrentUpdateError, LifeWeaveRepository
from src.lifeweave.service import LifeWeaveService


class MemoryRepository:
    def list_discussions(self, workspace, item_id=None, entity_id=None):
        return []
...
def service() -> tuple[LifeWeaveService, MemoryRepository]:
    repo=MemoryRepository(); return LifeWeaveService(repo), repo  # type: ignore[arg-type]
...
class AtomicCreateConnection:
    ...
    def execute(self, statement: str, _: object = None) -> None:
        if 't_lifeweave_item' in statement: self.inserted.append('item')
        if 't_lifeweave_context' in statement: raise RuntimeError('context write failed')
...
def test_item_and_initial_context_roll_back_together() -> None:
    postgres=AtomicCreatePostgres(); repository=LifeWeaveRepository(postgres)  # type: ignore[arg-type]
