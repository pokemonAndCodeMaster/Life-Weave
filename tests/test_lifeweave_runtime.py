...
from src.lifeweave_runtime.worker import LifeWeaveWorker, ServiceWorkerClient
...
class FakeLifeWeave:
    def execution_feedback(self, workspace, item_id):
        return []
...
def service(tmp_path: Path, executor: ShellExecutor, *, repository_root: Path | None = None):
    ...
    runtime = LifeWeaveRuntimeService(
        repository=repo,  # type: ignore[arg-type]
        lifeweave_service=core,
...
def register(runtime: LifeWeaveRuntimeService, workspace: str = "personal", *, capacity: int = 1,
...
def test_child_delegation_prompt_keeps_local_goal_and_pins_parent_context(tmp_path: Path) -> None:
    from copy import deepcopy
    from test_lifeweave import service as core_service
    ...
    runtime.lifeweave_service = core
    first = runtime.create_run(
...
def test_docker_request_reports_unavailable_when_runtime_does_not_exist(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "src.lifeweave_runtime.worker.shutil.which",
